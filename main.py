...
from logger import init_logger, LOG_INFO, LOG_ERROR, LOG_DEBUG
    ...
    while not globals.stop_threads:
        ...
        if announcement_coin and \
            ...
            LOG_INFO(
                ...
                telegram_key='COIN_ANNOUNCEMENT')
            ...
            if supported_currencies:
                if announcement_coin in supported_currencies:
                    ...
                    try:
                        ...
                        if order_status == "closed":
                            ...
                            if order_status == "cancelled" and float(order[announcement_coin]['_amount']) > float(order[announcement_coin]['_left']) and float(order[announcement_coin]['_left']) > 0:
                                ...
                                LOG_INFO(f"Partial fill order detected.  {order_status=} | {partial_amount=} out of {amount=} | {partial_fee=} | {price=}")
                    ...
                    LOG_ERROR(
                        ...
                        telegram_key='COIN_NOT_SUPPORTED')
            ...
            LOG_INFO( 'No coins announced, or coin has already been bought/sold. Checking more frequently in case TP and SL need updating')
        time.sleep(3)
    ...
    while not globals.stop_threads:
        ...
        if len(order) > 0:
            for coin in list(order):
                ...
                LOG_DEBUG(f'Data for sell: {coin=} | {stored_price=} | {coin_tp=} | {coin_sl=} | {volume=} | {symbol=} ')
                ...
                LOG_INFO(f'{symbol=}-{last_price=}\t[STOP: ${"{:,.5f}".format(stop_loss_price)} or {"{:,.2f}".format(coin_sl)}%]\t[TOP: ${"{:,.5f}".format(top_position_price)} or {"{:,.2f}".format(coin_tp)}%]\t[BUY: ${"{:,.5f}".format(stored_price)} (+/-): {"{:,.2f}".format(((float(last_price) - stored_price) / stored_price) * 100)}%]')
                        ...
                        if not globals.test_mode:
                            ...
                            if sell._status != 'closed':
                                ...
                                if float(sell._left) > 0 and float(sell._amount) > float(sell._left):
                                    ...
                                    LOG_INFO(f"Sell order did not close! {sell._left} of {coin} remaining. Adjusted order _amount and _fee to perform sell of remaining balance")
